 ...
 */
class Zita_Site_Llibrary_Am_LicenseChecker 
{
    ...
    // successful code
    const ZITA_SITE_LIBRARY_OK = 'ok';
    const ZITA_SITE_LIBRARY_CONNECTION_ERROR = 'connection_error';
    // license error codes
    const ZITA_SITE_LIBRARY_LICENSE_EMPTY = 'license_empty';
    const ZITA_SITE_LIBRARY_LICENSE_NOT_FOUND = 'license_not_found';
    const ZITA_SITE_LIBRARY_LICENSE_DISABLED = 'license_disabled';
    const LICENSE_EXPIRED = 'license_expired';
    const ZITA_SITE_LIBRARY_LICENSE_SERVER_ERROR = 'license_server_error';
    // activation/deactivation problem codes
    const ZITA_SITE_LIBRARY_ACTIVATION_SERVER_ERROR = 'activation_server_error';
    const ZITA_SITE_LIBRARY_ERROR_INVALID_INPUT = 'invalid_input';
    const ZITA_SITE_LIBRARY_ERROR_NO_SPARE_ACTIVATIONS = 'no_spare_activations';
    const ZITA_SITE_LIBRARY_ERROR_NO_ACTIVATION_FOUND = 'no_activation_found';
    const ZITA_SITE_LIBRARY_ERROR_NO_REACTIVATION_ALLOWED = 'no_reactivation_allowed';
    const ZITA_SITE_LIBRARY_ERROR_NO_RESPONSE = 'no_response';
    const ZITA_SITE_LIBRARY_ERROR_OTHER = 'other_error';
    
    public $messages = array(
        self::ZITA_SITE_LIBRARY_OK => 'License OK',
        self::ZITA_SITE_LIBRARY_CONNECTION_ERROR => 'Could not connect to licensing server - please try again later',
        self::ZITA_SITE_LIBRARY_LICENSE_EMPTY => 'Empty or invalid license key submitted',
        self::ZITA_SITE_LIBRARY_LICENSE_NOT_FOUND => 'License key not found on licensing server',
        self::ZITA_SITE_LIBRARY_LICENSE_DISABLED => 'License key has been disabled',
        self::LICENSE_EXPIRED => 'License key expired',
        self::ZITA_SITE_LIBRARY_LICENSE_SERVER_ERROR => 'License server is not available - please try again later',
        self::ZITA_SITE_LIBRARY_ACTIVATION_SERVER_ERROR => 'Activation server error',
        self::ZITA_SITE_LIBRARY_ERROR_INVALID_INPUT => 'Activation failed: invalid input',
        self::ZITA_SITE_LIBRARY_ERROR_NO_SPARE_ACTIVATIONS => 'No more activations allowed',
        self::ZITA_SITE_LIBRARY_ERROR_NO_ACTIVATION_FOUND => 'No activation found for this installation',
        self::ZITA_SITE_LIBRARY_ERROR_NO_REACTIVATION_ALLOWED => 'Re-activation is not allowed',
        self::ZITA_SITE_LIBRARY_ERROR_NO_RESPONSE => 'Internal problem on activation server',
        self::ZITA_SITE_LIBRARY_ERROR_OTHER => 'Error returned from activation server',
    );

    protected $api_version = 1;
    /** @var int last code returned from */
    protected $code = self::ZITA_SITE_LIBRARY_OK;
    ...
    function checkLicenseKey()
    {
        $body = $this->makeRequest('GET', 'check-license', 
                array( 'key' => $this->key,), 
                self::ZITA_SITE_LIBRARY_LICENSE_SERVER_ERROR);
        ...
        return $this->code === self::ZITA_SITE_LIBRARY_OK;
    }
    ...
    function activate(& $activation_cache )
    {
        ...
        $this->activation_response = $this->processActivationResponse(
                ...
                self::ZITA_SITE_LIBRARY_ACTIVATION_SERVER_ERROR));
    ...
    function makeRequest($method, $action, $params, $errorCode, $responseClass = 'stdclass')
    {
        ...
        if ($status != 200)
        {
            $this->setError(self::ZITA_SITE_LIBRARY_CONNECTION_ERROR);
            return false;
        }
    ...
    protected function processActivationResponse($resp)
    {
        ...
        $response->return = $resp->code == self::ZITA_SITE_LIBRARY_OK;
        if ($this->call_home_days > 0)
            $response->next_check = min($response->next_check, 
                    $this->call_home_days * 3600 * 24);
        switch ($response->code)
        {
            case self::ZITA_SITE_LIBRARY_OK:
                break;
            case self::ZITA_SITE_LIBRARY_ACTIVATION_SERVER_ERROR:;
            case self::ZITA_SITE_LIBRARY_CONNECTION_ERROR:;
...
class Am_Crypt_Blowfish
{
    ...
    function _init()
    {
        $defaults = new Zita_Site_Library_Am_Crypt_Blowfish_DefaultKey();
        $this->_P = $defaults->P;
        $this->_S = $defaults->S;
    }
...
class Zita_Site_Library_Am_Crypt_Blowfish_DefaultKey
{
    var $P = array();
